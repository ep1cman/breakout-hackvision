...
boolean nunchuk;          //If a nunchuk is available
...
void movePaddle()
{
  ...
  if(xPaddle < width-12)
  {
    if(Controller.rightPressed() || (nunchuk && (Nunchuk.getAccelerometerX() > 150)) || (nunchuk && (Nunchuk.getJoystickX() > 200)))
    {
      xPaddle++;
    }
  }
  ...
  if(xPaddle > 0)
  {
    if(Controller.leftPressed() || (nunchuk && (Nunchuk.getAccelerometerX() < 100)) || (nunchuk && (Nunchuk.getJoystickX() < 100)))
    {
      xPaddle--;
    }
  }
...
void enterInitials() {
  ...
  while (true) {
    ...
    TV.delay(150);
    Nunchuk.getData();
    if (Controller.leftPressed() || (nunchuk && (Nunchuk.getJoystickX() < 100))) {
      index--;
      if (index < 0) {
	index = 0;
      } else {
    ...
    if (Controller.rightPressed() || (nunchuk && (Nunchuk.getJoystickX() > 200))) {
      index++;
      if (index > 2) {
	index = 2;
      } else {
    ...
    if (Controller.upPressed() || (nunchuk && (Nunchuk.getJoystickY() > 200))) {
      initials[index]++;
      TV.tone(523, 20);
      // A-Z 0-9 :-? !-/ ' '
      if (initials[index] == '0') {
    ...
    if (Controller.downPressed() || (nunchuk && (Nunchuk.getJoystickY() < 100))) {
      initials[index]--;
      TV.tone(523, 20);
      if (initials[index] == ' ') {
	initials[index] = '?';
